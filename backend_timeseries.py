...
from frontend_timeseries import initialize_plot, app, win
...
data = np.zeros((9, 0))  # 9 channels including sync
...
# The above is mostly taken from your code with bokeh but abstracted into smaller functions
...
def stream():
    ...
    global data_loaded, eeg, filter_params, notch_on
    global dirname

    import frontend_timeseries
    import subprocess

    screen = frontend_timeseries.screen
    ...
    # Load values from frontend screen settings
    min_freq, max_freq = frontend_timeseries.min_freq, frontend_timeseries.max_freq  # Import min_freq and max_freq from frontend
    time_lapsed = frontend_timeseries.time_lapsed

    # Update axis for the graphs based on the frontend screen settings
    for i in range(8):
        plots[i].setYRange(min_freq, max_freq)
        plots[i].setXRange(0, time_lapsed)

    # The below code is mostly taken from your code with bokeh
    datastamp += 1
    ...
    if data.shape[1] == 0:
        ...
        if not os.path.exists(os.path.join(dirname, 'stop.txt')):
            eeg_counter += 1
            data_loaded = eeg.get_recording_no_time(include_timestamp=True)
            np.save(os.path.join(dirname, str(eeg_counter) + '.npy'), data_loaded)
            print('EEG Data Collection: Saved {}'.format(str(eeg_counter) + '.npy'))
            print('EEG Data Collection: Shape of data_loaded = {}'.format(data_loaded.shape))
        else:
            eeg.stop_stream()
    ...
    # Update the curves plotted based on the new data
    xs = xs*25000
    xs = xs.astype(int)
